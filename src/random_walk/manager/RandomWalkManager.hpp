...
public:
     ...
     * @param walks_per_vertex The number of walks to generate per vertex.
    ...
    RandomWalkManager(const IGraphManager<GraphTraits>& graph, int num_threads,
                      ...
                      int walk_length, int walks_per_vertex = 10, int seed = 0);

    /**
     * @brief Generate random walks for a set of vertices.
     * @note NOT thread-safe if called concurrently
     *
...
private:
    ...
    const int m_walks_per_vertex; // The number of walks to generate per vertex
