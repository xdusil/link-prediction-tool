...
std::tuple<RandomForestParams, double> RandomForestClassifier<Features, Labels, AverageStrategy>::grid_search(const Features &features, const Labels &labels,
    ...
    if (num_trees.empty() || min_leaf_size.empty() || min_gain_split.empty() || max_depth.empty()) {
        throw RandomForestException("Grid search requires non-empty hyperparameter lists.");
    }

    arma::mat double_features;
    if constexpr (std::is_same_v<Features, arma::fmat>) {
        std::cout << "Converting features from float to double precision for grid search..." << std::endl;
